...
\maketitle
\noindent\makebox[\textwidth]{9 August 2020} % FIXME
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$1 \leq i \leq \lceil (2\bitlength(m)-1)/k \rceil$}
            ...
            \State{$\bar a \leftarrow (a \bmod 2^k) + 2^k \lfloor a / 2^{n-k} \rfloor$}
                \Comment{$\bar a < 2^{2k}$}
            \State{$\bar b \leftarrow (b \bmod 2^k) + 2^k \lfloor b / 2^{n-k} \rfloor$}
                \Comment{$\bar b < 2^{2k}$}
            ...
            \EndFor
            \State{$a \leftarrow af_0 + bg_0$}
            \If{$a < 0$}
                \State{$(a, f_0, g_0) \leftarrow (-a, -f_0, -g_0)$}
            \EndIf
            \State{$u \leftarrow uf_0 + vg_0 \bmod m$}
            \State{$b \leftarrow af_1 + bg_1$}
            \If{$b < 0$}
                \State{$(b, f_1, g_1) \leftarrow (-b, -f_1, -g_1)$}
            \EndIf
            \State{$v \leftarrow uf_1 + vg_1 \bmod m$}
        \EndFor
        \State{$v \leftarrow v/2^{k\lceil (2\bitlength(m)-1)/k \rceil} \bmod m$}
        \If{$b \neq 1$}
            \State{\Return 0 (value $y$ is not invertible)}
        \EndIf
        \State{\Return $v$}
